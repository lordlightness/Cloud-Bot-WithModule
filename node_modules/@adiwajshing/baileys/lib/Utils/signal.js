...
const signalStorage = ({ creds, keys }) => ({
    ...
    getOurIdentity: () => {
        ...
        return {
            ...
            pubKey: crypto_1.generateSignalPubKey(signedIdentityKey.public),
        };
    }
});
...
const getNextPreKeys = async ({ creds, keys }, count) => {
    const { newPreKeys, lastPreKeyId, preKeysRange } = exports.generateOrGetPreKeys(creds, count);
    const update = {
        nextPreKeyId: Math.max(lastPreKeyId + 1, creds.nextPreKeyId),
        firstUnuploadedPreKeyId: Math.max(creds.firstUnuploadedPreKeyId, lastPreKeyId + 1)
    };
    await keys.set({ 'pre-key': newPreKeys });
    const preKeys = await exports.getPreKeys(keys, preKeysRange[0], preKeysRange[0] + preKeysRange[1]);
    return { update, preKeys };
};
exports.getNextPreKeys = getNextPreKeys;
const getNextPreKeysNode = async (state, count) => {
    const { creds } = state;
    const { update, preKeys } = await exports.getNextPreKeys(state, count);
    const node = {
        tag: 'iq',
        attrs: {
            xmlns: 'encrypt',
        ...
        content: [
            ...
            { tag: 'list', attrs: {}, content: Object.keys(preKeys).map(k => exports.xmppPreKey(preKeys[+k], +k)) },
            exports.xmppSignedPreKey(creds.signedPreKey)
        ]
    };
    return { update, node };
};
